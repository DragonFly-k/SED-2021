...
namespace GRB
{
	Greibach greibach(NS('S'), TS('$')				// стартовый символ / дно стека
		, 7
		, Rule(NS('S'), GRB_ERROR_SERIES + 0, // структура программы
			3,
		...
		, Rule(NS('F'), GRB_ERROR_SERIES + 4, // формальные параметры функции
			2		
			, Rule::Chain(2, TS('t'), TS('i'))
			, Rule::Chain(4, TS('t'), TS('i'), TS(','), NS('F')))

		, Rule(NS('W'), GRB_ERROR_SERIES + 5,//	фактические параметры функции
			4		
			, Rule::Chain(1, TS('i'))
			, Rule::Chain(1, TS('l'))
		...
		, Rule(NS('Q'), GRB_ERROR_SERIES + 6, //	Условие условного оператора
			4		
