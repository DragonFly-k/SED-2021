...
#define PARM_OUT_DEFAULT_EXT L".asm" //расширение файла обьектного кода
...
namespace Parm //обработка входных параметров
{
	struct PARM //входные параметры
	{
		...
		wchar_t out[PARM_MAX_SIZE];  //-out: имя файла обьектного кода
		wchar_t log[PARM_MAX_SIZE];  //-log: имя файла протокола
	};

	PARM getparm(int argc, wchar_t* argv[]); //структура Parm на основе параметров функции main
}
