...
namespace Semantic
{
	void SemanticAnalysis(Lex::Tables* tables)
	{
		...
		cout << "Семантический анализ выполнен" << endl;
	}

	void CheckTypesMapping(Lex::Tables* tables)// проверка на совпадение типов
	{
		...
		for (int i = 0; i < tables->lexTable.size; i++)
		{
			...
			{
				if (tables->idenTable.table[tables->lexTable.table[i - 1].idxTI].idtype != IT::V && tables->idenTable.table[tables->lexTable.table[i - 1].idxTI].idtype != IT::P)throw ERROR_THROW_IN(SEM_ERROR_SIRES + 0, tables->lexTable.table[i - 1].sn, 0);		//плохое значение
				...
				case IT::STR:
				{
					...
					{
						if (tables->lexTable.table[i + j].lexema == LEX_ARITHMETIC)	//попытка сделать арифметическую операцию со строкой
						{
						...
						if (tables->lexTable.table[i + j].lexema == LEX_ID || tables->lexTable.table[i + j].lexema == LEX_LITERAL || tables->lexTable.table[i + j].lexema == LEX_CONCAT|| tables->lexTable.table[i + j].lexema == LEX_STRLEN)
						{
							if (tables->idenTable.table[tables->lexTable.table[i + j].idxTI].idtype == IT::F)
							{
								...
								{
									throw ERROR_THROW_IN(SEM_ERROR_SIRES + 2, tables->lexTable.table[i + j].sn, 0);	//плохой вызов функции
								}
							...
							{
								throw ERROR_THROW_IN(SEM_ERROR_SIRES + 3, tables->lexTable.table[i + j].sn, 0);//Неучтенные типы в выражении
							}			
						}
					}
					break;
					...
					while (tables->lexTable.table[i + index].lexema != LEX_SEMICOLON)
					{
						if (tables->lexTable.table[i + index].lexema == LEX_LITERAL || tables->lexTable.table[i + index].lexema == LEX_ID || tables->lexTable.table[i + index].lexema == LEX_CONCAT|| tables->lexTable.table[i + index].lexema == LEX_STRLEN)
						{
							if (tables->idenTable.table[tables->lexTable.table[i + index].idxTI].idtype == IT::IDTYPE::F)
							{
								...
								{
									throw ERROR_THROW_IN(SEM_ERROR_SIRES + 3, tables->lexTable.table[i + index].sn, 0);	   //Неучтенные типы в выражении
								}

								if (tables->lexTable.table[i + index + 1].lexema != LEX_LEFTHESIS)
									throw ERROR_THROW_IN(SEM_ERROR_SIRES + 2, tables->lexTable.table[i + index].sn, 0);	// попытка вызова функции без ()
	...
	void CheckCondAndStreams(LT::LexTable& lextable, IT::IdTable& idtable)
	{
		for (int i = 0; i < lextable.size; i++)
		{
			if (lextable.table[i].lexema == LEX_WRITE)				// проверка корректности write
			{
				int j = i;
			...
			if (lextable.table[i].lexema == LEX_IF)	// проверка корректности if-else 
			{
