...
void Generation::CodeGeneration(Lex::Tables& tables)
{
	ofstream fout;
	string funcName = "";
	stack<int> stk;
	stack<int> ifI;
	short numOfret = 0, numOfIf = 0;
	bool flagFunc = false, flEntry = false, flret = false, flOut = false, flElse = false, flIf = false;
	fout.open("D:\\лабы\\€п\\курсач\\SED-2021\\SED2021.asm");
	...
	fout.close();
	std::cout << "√енераци€ кода выполнена" << endl;
}
