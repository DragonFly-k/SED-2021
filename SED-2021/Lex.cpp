...
namespace Lex
{
	...
	Tables LexAnalysis(Words* tokens)
	{
		...
		for (int i = 0; i < tokenCount; i++)
		{
			...
			if (resAn == '\0')
			{
				...
				else if (CheckId(tokens))
				{
					...
					{
						if (!(CHECKFORDECLARE) && !isPar && !(LIBFOO))throw ERROR_THROW_IN(126, LINE, SYM);		// использование незаявленного id
					...
					else if (CHECKFORDECLARE)throw ERROR_THROW_IN(130, LINE, SYM);		// переопределение идентификатора
				}
			}
			else
			{
		...
		cout << "Лексический анализ выполнен" << endl;
		return table;
	}
	...
	void AddID(char* token, Tables* tables, IT::Entry tempIT)
	{
		if (isPar)
		{
			if (!(CHECKPARM))throw ERROR_THROW_IN(127, LEXLINE, LEXPOS);		// плохое объявление параметров
			tempIT.idtype = IT::P;
		}
	...
	char CheckForArithmAndBrack(char* lexem, Tables* tables)
	{
		...
		case LEX_SEMICOLON:
		{
			...
			{
				int brBalance = 1;
				for (int i = tables->lexTable.size - 2; i != -1; i--) //проверка на кол закрытых скобок
				{
				...
				if (brBalance != 0)throw ERROR_THROW_IN(116, LEXLINE, LEXPOS);		//неверное количество скобок
				parrentBlock.pop();
			}
	...
	void AnalysisItString(IT::Entry* tempIT, char& lexem, Words* tokens)
	{
		...
		case LEX_START:
		{
			if (mainIsDeclared)throw ERROR_THROW_IN(125, LINE, SYM);		//проверка на множественное обьявление start
			mainIsDeclared = true;
	...
	bool CheckForStringLiteral(Words* tokens)
	{
		...
		if (lex[0] == '\'')
		{
			if (strlen(lex) > 100)throw ERROR_THROW_IN(120, LINE, SYM);		//слишком длинный литерал
			return true;
		}
		return false;
	}
	...
	bool CheckForIntegerLiteral(Words* tokens)
	{
		...
		if ((atoll(lex) > pow(2, 31)) || (atoll(lex) < pow(-2, 31)))throw ERROR_THROW_IN(123, LINE, SYM); // слишком большое значение
		return true;
	}
	...
	bool CheckId(Words* tokens)
	{
		char* lex = tokens->arr.front().token;
		int len = strlen(lex);
		if (len > ID_MAXSIZE)throw ERROR_THROW_IN(121, LINE, SYM);							// слишком длинный
		for (int i = 0; i < len; i++) 
		{
          if (lex[i] < 'a' || lex[i] > 'z')throw ERROR_THROW_IN(122, LINE, SYM);			// плохое объявление id
		}		
		return true;
	}
