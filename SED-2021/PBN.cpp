...
namespace PN
{
	...
	void ShiftAndWriteLT(Lex::Tables& tables, Pos* pos, int newLen)
	{
		for (int i = pos->startEx, j = 0; j < newLen; i++, j++)
		{
			...
			if (tables.lexTable.table[i].lexema == LEX_ID || tables.lexTable.table[i].lexema == LEX_LITERAL || tables.lexTable.table[i].lexema == LEX_CONCAT|| tables.lexTable.table[i].lexema == LEX_STRLEN)
			{
		...
		for (int i = pos->endEx - (pos->startEx + newLen), j = 0; j < i; j++)
		{
			for (int k = pos->startEx + newLen; k < tables.lexTable.size; k++)
			{
				...
				if (tables.lexTable.table[k].lexema == LEX_ID || tables.lexTable.table[k].lexema == LEX_LITERAL || tables.lexTable.table[k].lexema == LEX_CONCAT|| tables.lexTable.table[k].lexema == LEX_STRLEN)
				{
